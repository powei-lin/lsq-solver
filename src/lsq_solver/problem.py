...
from .loss_funcs import make_loss
from .auto_diff import make_jac
...
class LeastSquaresProblem:
    ...
    def bound_variable(self, variable: np.ndarray, min=-np.inf, max=np.inf):
        ...
        self._address_bounds_map[address] = (min, max)
    ...
    def _batch_jac(self, x):
        ...
        :return:
        """
        J = dok_matrix((self._dim_residual, self._dim_variable), dtype=np.float64)
        for residual_block in self._residual_blocks:
            variables = []
            for col_range in residual_block.col_ranges:
                variables.append(x[col_range[0]: col_range[1]])
    ...
    def _batch_loss(self, z):
        ...
        """
        assert z.shape[0] == self._dim_residual
